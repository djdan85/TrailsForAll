...
export default function Admin() {
  ...
  const fetchAll = async () => {
    const { data: trailsData } = await supabase
      .from('trails_with_profiles').select('*').order('created_at', { ascending: false })
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-6">
          <div className="bg-gray-900 rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-orange-500">{trails.filter(t => t.status === 'pending').length}</p>
            <p className="text-gray-400 text-xs mt-0.5">Ke schválení</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-green-400">{trails.filter(t => t.status === 'approved').length}</p>
            <p className="text-gray-400 text-xs mt-0.5">Schválených</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-yellow-400">{reviews.filter(r => r.status === 'pending').length}</p>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      {article.category && (
                        <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${categoryColor[article.category]}`}>
                          {categoryLabel[article.category]}
                        </span>
                      )}
                      <span className={`text-xs px-3 py-1 rounded-full font-semibold ${
                        article.status === 'pending' ? 'bg-yellow-900 text-yellow-400' :
                        article.status === 'published' ? 'bg-green-900 text-green-400' :
                        article.status === 'draft' ? 'bg-gray-700 text-gray-400' :
                        article.status === 'deleted' ? 'bg-red-900 text-red-400' :
                  ...
                  {editingTrail?.id === trail.id ? (
                    <div className="p-6 flex flex-col gap-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                          ...
                          <input className="w-full bg-gray-800 text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-orange-500 text-sm" value={editingTrail.name} onChange={e => setEditingTrail({...editingTrail, name: e.target.value})} />
                        </div>
                        <div>
                          <label className="text-gray-400 text-xs mb-1 block">Lokalita</label>
                          <input className="w-full bg-gray-800 text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-orange-500 text-sm" value={editingTrail.location_name} onChange={e => setEditingTrail({...editingTrail, location_name: e.target.value})} />
                        </div>
                      </div>

                      <div>
                        <label className="text-gray-400 text-xs mb-1 block">Popis</label>
                        <textarea className="w-full bg-gray-800 text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-orange-500 text-sm" rows={3} value={editingTrail.description} onChange={e => setEditingTrail({...editingTrail, description: e.target.value})} />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <input className="w-full bg-gray-800 text-white rounded-xl px-4 py-2.5 outline-none focus:ring-2 focus:ring-orange-500 text-sm" type="number" value={editingTrail.length_km} onChange={e => setEditingTrail({...editingTrail, length_km: e.target.value})} />
                        </div>
                        <div>
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex gap-2 flex-wrap mb-3">
                        <span className="bg-gray-800 text-gray-300 text-xs px-2 py-1 rounded-lg">{trailTypeLabel[trail.trail_type] || '🚵 Singltrek'}</span>
                        <span className="bg-gray-800 text-gray-300 text-xs px-2 py-1 rounded-lg">{skillLevelLabel[trail.skill_level || trail.difficulty] || '🟢 Začátečník'}</span>
                        <span className="bg-gray-800 text-gray-300 text-xs px-2 py-1 rounded-lg">📏 {trail.length_km} km</span>
                        {trail.region && <span className="bg-gray-800 text-gray-300 text-xs px-2 py-1 rounded-lg">🗺️ {trail.region}</span>}
                      </div>
